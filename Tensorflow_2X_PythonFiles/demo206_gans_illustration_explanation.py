# -*- coding: utf-8 -*-
"""Demo206_GANs_Illustration_Explanation.ipynb
...
3.   Create the generator model 
     * g = Model(Noise, Image)
     * do not add binary cross entropy here 
4.   Combine the two models in cascade; the generator output image should be the discriminator input image 
     * fake_image_prediction = d(g(noise)) = d(image_by_generator) 
     * combined = Model(noise, fake_image_prediction) 
     * combined model loss will be binary cross entropy but the target variable is always 1 because the generator needs the loss of fake_image_prediction with 1, as 1 signifies a real image 

5.   Run a backpropagation loop to train discriminator that does the following for each epochs:
     * get real_images
     * get fake images = generator.predict(noise) 
     * d.train(real_images, ones)
     * d.train(fake_images, zeros)

6.   To train the generator, freeze discriminator weights, and train:
     * combined_model.train(fake_images, ones)
"""
