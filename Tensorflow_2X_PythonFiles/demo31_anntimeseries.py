...
n = 1001
x = 0.1 * np.arange(1,n)
data = np.cos(x) + np.random.randn(n-1) * 0.4
...
y = []
print(len(data))  
print("The last sample would be: 1000 - 20 = " + str((len(data)-T)) )
...
index = -N//4
report = model.fit(X[:index], y[:index], epochs=500, validation_data=(X[index:], y[index:]))
